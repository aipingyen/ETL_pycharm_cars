...
def get_car_urls(brand_list,file_out,page):
    headers = gen_headers()
    ...
    while count < 3:
        ...
        try:
            ...
            if count == 3:
                ...
                #exception直接印出
                print(message)
                with open('fail_page.csv','a')
        finally:
            time.sleep(0.2)

if __name__=='__main__':
    car_no = get_car_no('https://auto.8891.com.tw/usedauto-search.html?page=1')
    ...
    for page in range(1, page_no + 1):
        ...
        if len(temp_list) == 4 and page_no >= 4:
            ...
            pool = multiprocessing.Pool(processes=8)
            res = pool.map(get_car_urls(brand_list, fileOut, page), temp_list)
            pool.close()

        #測試時car_no可以自己填100(只爬100輛車)
        get_car_urls(brand_list, fileOut, page)
